...
def get_activities_count(
    ...
    """Returns the exact total count of activities for THIS user's contacts only."""
    user_contact_ids = db.query(Contact.id).filter(Contact.user_id == current_user.id).subquery()
...
def list_recent_activities(
    ...
    """Returns recent activities across all of the current user's contacts."""
    return get_recent_activities(db, current_user.id, limit=limit)
